...
export default function AdminComments() {
    ...
    async function setStatusFor(id, s) {
        await api.patch(`/admin/comments/${id}/status`, { status: s })
        refresh()
    }
                ...
                <div className="card-body p-0">
                    <div className="table-responsive">
                        <table className="table table-hover align-middle mb-0">
                            <thead><tr><th>Listing</th><th>Content</th><th>Status</th><th></th></tr></thead>
                            <tbody>
                                {items.map(r => (
                                    <tr key={r._id}>
                                        <td>{r.listing}</td>
                                        <td className="text-truncate" style={{ maxWidth: 480 }}>{r.content}</td>
                                ...
                                {!items.length && <tr><td colSpan={4} className="text-muted">No comments</td></tr>}
                            </tbody>
                        </table>
                    </div>
                </div>
